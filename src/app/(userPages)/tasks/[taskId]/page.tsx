...
const schema = yup.object().shape({
  ...
  taskDate: yup.date().required('Title date is required'),
  taskCategory: yup.object().required('Title category is required'),
  taskPriority: yup.number().required('Title priority is required'),
});
...
const Task = ({ params }: TaskIdPageProps) => {
  ...
  const [initialTaskCompletedAt, setInitialTaskCompletedAt] =
    useState<number>(0);
  const [initialTaskCategory, setInitialTaskCategory] =
  ...
  useEffect(() => {
    ...
    if (foundTask) {
      ...
      setInitialTaskCategory(foundTask.category as CategoryType);
      ...
      setPotentialCategory(foundTask.category as CategoryType);
      ...
      setCustomValue('taskDate', new Date(foundTask.todayAt));
      ...
      <footer className="mt-4 flex w-full items-center justify-between min-[500px]:gap-8">
        <Button
          ...
          onClick={() => {
            ...
            setPotentialDate(new Date(taskDate.getTime()));
            setSelectedDate(new Date(taskDate.getTime()));
    ...
    footerContent = (
      <footer className="mt-6 flex w-full items-center justify-between min-[500px]:gap-8">
        <Button
          label="Cancel"
          onClick={() => {
            setPotentialCategory(task.category as CategoryType);
            setIsOpen(false);
          }}
        />
        <Button
          ...
          onClick={() => {
            setTask((prev) => {
              const modifiedTask = prev as TaskType;
              modifiedTask.category = potentialCategory as CategoryType;
              return modifiedTask;
            });
  ...
  const onRecoverInitialData = () => {
    setTask({
      ...
      category: {
        icon: initialTaskCategory?.icon as string,
        iconBgColor: initialTaskCategory?.iconBgColor as string,
        iconColor: initialTaskCategory?.iconColor as string,
  ...
  return (
    <Container>
      <div className="mx-auto flex min-h-screen max-w-lg flex-col justify-between gap-4 pb-12 pt-9">
        <div>
          <header className="relative flex items-center justify-between">
            <div className="group">
              <div
                onClick={() => onRecoverInitialData()}
          ...
          <div className="mt-8">
            <div className="flex flex-col gap-9">
              <div className="flex items-center justify-between">
                <div className="flex gap-3">
                  <ClockIcon
                    onClick={() => {}}
                    customClasses={
                      'text-gray-dark dark:text-white-pale w-7 h-7'
                    }
                  />
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {task.category.icon.includes('Md')
                    ? AllMdIcons[task.category.icon as keyof typeof AllMdIcons](
                        {
                          color: task.category.iconColor,
                          className: 'w-7 h-7 min-[500px]:w-9 min-[500px]:h-9',
                        },
                      )
                    : AllIo5Icons[
                        task.category.icon as keyof typeof AllIo5Icons
                      ]({
                        color: task.category.iconColor,
                        className: 'w-7 h-7 min-[500px]:w-9 min-[500px]:h-9',
                      })}
        ...
        <footer className="mx-auto mt-16 flex w-full max-w-lg items-center justify-center min-[500px]:gap-14">
          <Button
            type="submit"
            label="Edit Task"
            onClick={() => onBack('edit')}
