  ...
  <div className="mb-6 flex items-center justify-between rounded-lg border border-gray-600 bg-gray-light px-5 py-4 dark:border-gray-700 dark:bg-gray-500">
    <div className="flex w-full items-center justify-between">
      <div className="flex items-center">
        <div
          className="mr-5 flex justify-center p-1"
          style={{ backgroundColor: bgIconColor }}
        >
          {icon.includes('Md')
            ? AllMdIcons[icon as keyof typeof AllMdIcons]({
                color: iconColor,
                className: 'w-7 h-7 min-[500px]:w-9 min-[500px]:h-9',
              })
            : AllIo5Icons[icon as keyof typeof AllIo5Icons]({
                color: iconColor,
                className: 'w-7 h-7 min-[500px]:w-9 min-[500px]:h-9',
              })}
        </div>
        <div>
          ...
          <p className="text-sm text-white-pale min-[500px]:text-lg">{text}</p>
        </div>
      </div>
